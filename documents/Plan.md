...
1. Успешная оплата валидной картой, со статусом APPROVED.
    ...
    * в таблице payment_entity появляется запись, значение поля status - APPROVED;
...
3. Отказ в оплате с карты, со статусом DECLINED.
    ...
    * в таблице payment_entity появляется запись, значение поля  status - DECLINED;
...
## Перечень используемых инструментов
  ...
  **Faker** -  это библиотека, для генерации данных (например ФИО).
